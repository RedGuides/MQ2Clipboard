...
#include "ISXEQClipboard.h"

// The mandatory pre-setup function.  Our name is "ISXEQClipboard", and the class is ISXEQClipboard.
...
// our instanced class.
ISXPreSetup("ISXEQClipboard", ISXEQClipboard);
...
unsigned int ISXEQClipboardXML = 0;
...
bool ISXEQClipboard::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQClipboard Loaded");
	return true;
}

// shutdown sequence
void ISXEQClipboard::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQClipboardXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQClipboardXML);
...
void ISXEQClipboard::ConnectServices()
{
...
void ISXEQClipboard::RegisterCommands()
{
	// add any commands
...
void ISXEQClipboard::RegisterAliases()
{
	// add any aliases
}

void ISXEQClipboard::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQClipboard::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQClipboard::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQClipboard::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQClipboard::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQClipboard::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQClipboardService)
	//        pISInterface->ShutdownService(this,hISXEQClipboardService);
}

void ISXEQClipboard::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQClipboardXML
	ISXEQClipboardXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQClipboardXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQClipboardXML, "General"))
	{
